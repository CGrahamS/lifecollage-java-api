    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        try {
            ...
            if (url.contains("/api/")) {
                ...
                logger.debug(uuid + " Origin request completed in " + (DateTime.getEpochMillis() - start) + "ms at " + DateTime.nowToString());
            } else {
                filterChain.doFilter(servletRequest, servletResponse);
            }

        } catch (Exception ex) {
            if (ex.getMessage() != null && ex.getMessage().contains("com.fasterxml.jackson.databind.JsonMappingException")) {
